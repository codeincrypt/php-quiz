...
<?php $qry = $conn->query("SELECT * FROM quiz_list where id = " . $_GET['id'])->fetch_array(); ?>
</head>
...
<body>
	...
	<div class="container-fluid admin">
		<div class="col-md-12 alert alert-primary">QUIZ : <?php echo $qry['title'] ?></div>
		...
		<div class="card col-md-6 mr-4" style="float:left">
			...
			<div class="card-body">
				<ul class="list-group">
					<?php
					$qry2 = $conn->query("SELECT * FROM questions where qid = " . $_GET['id'] . " order by order_by asc");
					while ($row = $qry2->fetch_array()) {
					?>
						<li class="list-group-item"><?php echo $row['question'] ?>
							<br><br>
							<?php $qry3 = $conn->query("SELECT * FROM question_opt where is_right = '1' AND question_id = " . $row['id'])->fetch_array(); ?>
							<b>Correct Answer</b> : <?php echo $qry3 ? $qry3['option_txt'] : "null" ?>
							<br><br>	
